...
2. **策略模式 (Strategy Pattern)**：将每一种输出渠道封装为独立的策略类（如 `ConsoleTransport`）。系统允许在运行时动态挂载多个输出策略，实现同一份日志向多渠道分发。
3. **结构化数据流转**：彻底摒弃在 Logger 层拼接字符串的落后做法。核心层仅负责组装结构化的 `LogEntry` 对象，将“如何展示”的决定权下放给底层的各个 Transport。
